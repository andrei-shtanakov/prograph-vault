...
- ~~Blockers in the transitional `<repo>#<number>` form are matched textually; their *state* (issue closed = delivered) is not yet resolved by the sensor.~~
  ...
  devtools#41, `blocker-issue-state-resolution`) — a closed inbox issue behind
  an open item raises the same stale-blocker finding as a delivered `todo://`
  edge, and unresolvable state (API unavailable) reports as explicitly
  UNAVAILABLE, never as clean. The transitional form is thereby a full citizen
  of the return leg; the canonical `todo://` edge remains preferred where the
...
- ~~The signal currently surfaces only in a red scheduled run.~~
  ...
  the comparison baseline is missing. Deliberately narrow: a wait the author has
  already acted on raises no alarm. Issue-form blockers are covered by the two
  readers unevenly: the scheduled-run sensor resolves their issue state (bullet
  above, it runs devtools `check-plan-fields.py`), while the digest section
  computes locally from the shared `plan_fields` package, which matches
  issue-form refs textually only — so the digest's issue-form caveat stands
  until robin-runtime adopts issue-state resolution. The red scheduled run
  remains the machine evidence; the digest is the human read of the same
  verdict class.
