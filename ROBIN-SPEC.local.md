...
> Resolved: -------------------------------------------------------------------
> Resolved: LOCAL CONFIGURATION — generated by /robin-init on 2026-07-07.
...
> Resolved: '> Resolved:' line and the local Appendix to reconstruct the upstream file.
> Resolved: -------------------------------------------------------------------
...
> Resolved: Slot 6 (Read-only data sources): sibling repos (read-only); Linear, Gmail, Google Drive/Calendar via MCP.
> Resolved: Slot 7 (Chat identity registry): maintainer = Andrei; team channel TBD — set via /telegram:access (documented config, not knowledge).
> Resolved: Slot 8 (Passive-capture scope): the team Telegram working channel, disclosed to the team — confirm exact channel at deploy.
> Resolved: Slot 9 (Duty roster): declared in robin/duties.md — Ecosystem digest (MUST), Integration-health brief, AI-news digest; VoC + meetings deferred.
> Resolved: Slot 10 (Digest cadence + destination): weekly Mon 09:00 → team Telegram channel — configurator proposal; override in robin/duties.md.
> Resolved: Slot 11 (Staleness budget): 14 days (spec default; matches the KB's '>2 weeks = stale' convention).
> Resolved: Slot 12 (DM idle window): 60 minutes (spec default).
> Resolved: Slot 13 (Ambient context size N): 10 — configurator proposal (reference value).
> Resolved: Slot 14 (DM reseed turn count): 10 — configurator proposal.
> Resolved: Slot 15 (Context-isolation mechanism): dedicated Robin workdir; host MCP/memory/global instructions MUST NOT load — deferred to implementer with this default (§6.5).
> Resolved: Slot 16 (Feedback + promotion affordance): Telegram 👍/👎 → staged learning; maintainer-DM approve/reject with route (memory | skill | repo-PR) at review (§6.4).
> Resolved: Slot 17 (Secrets location): environment variables on the VPS (never in the KB) — spec default.
> Resolved: Slot 18 (Meeting-bot provider + auto-join): DEFERRED — no meeting-recorder/notetaker capture detected (§6.6 optional).
> Resolved: Slot 19 (Meeting transcription provider): DEFERRED — see slot 18.
> Resolved: Slot 20 (In-meeting invocation + transcript window): DEFERRED — see slot 18.
> Resolved: Slot 21 (Input modalities beyond text): text + images (voice notes deferred / transcribe-before-processing) — reference reads images.
> Resolved: Slot 22 (Team-skill consumption): thin pointers to authored/skills/*/SKILL.md over the synced mount, not copies; sync cadence daily (§4).
...
## Appendix (local, added by /robin-init) — Conformance Checklist

Generated 2026-07-07 for the AI-Orchestrators ecosystem against prograph-vault.
Upstream spec: ROBIN-SPEC.md @ git b5ad49e (sha256 f8f49c73…). Identity: ./soul.md.
Duty roster: ./robin/duties.md. This checklist substitutes the team's real recurring
question and declared duties into the spec's §8 milestones.

Acceptance tests (§8):

- [ ] M0 — Repo Q&A, local. From a fresh agent session, "Which repo owns the
      agents-catalog SSOT?" is answered `atp-platform/method/agents-catalog.toml`,
      citing `authored/decisions/2026-07-01-adr-eco-003-agent-catalog.md`
      (amendment 2026-07-03).
- [ ] M1 — Chat-native Q&A. A teammate other than Andrei asks the same question in the
      team Telegram channel and gets that repo-grounded answer with the citation. ← the
      demonstration event: this moment, not deployment, is when Robin starts existing.
- [ ] M2 — Scheduled duties (robin/duties.md). Each posts on schedule and persists:
      - Ecosystem digest (weekly Mon 09:00 → team channel); "what did I miss this
        week?" is answerable from it.
      - Integration-health / drift brief (weekly → team channel).
      - AI-news digest (daily → team channel), taken over from the existing pipeline.
- [ ] M3 — Ambient context. A group mention is answered from the last N=10 channel
      messages + recent digests, without re-explanation.
- [ ] M4 — Learning loop. A 👍/👎 correction becomes a staged file, verified persisted by
      read-back (§6.4), is human-promoted, and observably changes a later answer.
- [ ] M5 — Meetings (optional). DEFERRED: no meeting-recorder/transcription capture
      connected (slots 18–20).

Mandatory hardening (written from the reference's failures — do not skip):

- [ ] §6.4 read-back verification of every staged write.
- [ ] §7 negative-evidence: state-file staleness alert; memory-listing completeness assert.
- [ ] §7 liveness: alert when the newest digest is older than its cadence + grace window.
- [ ] §7 cost: per-day observability + a daily budget cap and per-user rate limit.
- [ ] §6.5 context isolation: no host MCP/memory/global instructions in Robin's context.
- [ ] §6.7 output escaping: escape user-derived content for Telegram's parser before
      formatting; a formatting-rejected send is a logged failure, not a silent fallback.
- [ ] §4 staleness tiers: honor `tier:`/`last_reviewed`; discount volatile > 14 days.
