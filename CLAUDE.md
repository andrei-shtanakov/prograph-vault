...
- **Ревью PR — терминальный прогон от ai-prosto** (дефолт с 2026-08-28):
  ...
  невалидное — ответить с обоснованием, не применять вслепую. **Stop rule**
  (решение владельца 2026-09-18): лимит — один полный review плюс один targeted
  recheck, и повторный круг открывает **только** находка, блокирующая по порогу
  гейта; `minor`/редактура не блокируют, лимит исчерпан — решает владелец. CI-гейт
  ...
  только по явной просьбе владельца. SSOT: `authored/rules/git-workflow.md`.
