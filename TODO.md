...
- После выполненной задачи — маркер task-list `- [ ] Подключить мета-энфорсер governance-плоскости: репо манифеста без каллера и каллер на устаревшем пине должны быть находкой @owner:github:andrei-shtanakov @id:governance-meta-enforcer — раскатка `governance-v2` 2026-08-20 вскрыла, что **ввод во флот и подключение к гейту — два разных ручных действия**: семь репо (devtools, kapelle, research-bench, impresario, disputatio, discovery-toolkit, robin-toolkit) числились в манифесте, но governance-каллера не имели вовсе, и заметить это было нечем. README зонтика описывает нужную проверку и помечает её **«not wired yet»**: «check-release-drift.py (или сосед) проверяет, что каллер каждого репозитория ссылается на канонический пин — так устаревший или отсутствующий гейт сам становится находкой дрейфа». Та же проверка поймала бы и **шаблон** каллера, который после создания v2 продолжал называть v1 (правлено вручную, зонтик PR #18). Пока энфорсера нет, любая раскатка протухает молча после каждого пополнения флота — сегодняшняя уборка на 23 PR повторится

- [x]` (именно так, не голое `[x]`:
